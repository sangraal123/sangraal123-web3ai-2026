...
// v6 Error Debugger DOM Elements
...
function setupEventListeners() {
  ...
  // v6 Error Debugger Event
  debugErrorBtn.addEventListener('click', handleDebugError);
}
...
// v6 Handle Error Debugging and Injected Roadmap Tasks
async function handleDebugError() {
  ...
  // Set loading state for debug button
  const debugText = debugErrorBtn.querySelector('.btn-text');
  const debugLoader = debugErrorBtn.querySelector('.btn-loader');
  
  debugText.textContent = 'エラー解決手順を生成中...';
  debugErrorBtn.disabled = true;
  if (debugLoader) debugLoader.classList.remove('hidden');
  ...
  if (!hasKey) {
    ...
    let mockAnalysis = "Node.js のパッケージ依存関係または実行設定に問題が発生している可能性があります。";
    let mockSteps = [
      { text: "node_modules ディレクトリを一度削除し、npm cache clean --force を実行する", timeEstimate: "5分" },
      { text: "npm install を再実行して依存パッケージをクリーンインストールする", timeEstimate: "5分" },
      { text: "npm run dev を実行して、ローカルサーバーが正常に起動するか確認する", timeEstimate: "2分" }
    ];

    if (errorText.toLowerCase().includes('git') || errorText.toLowerCase().includes('push')) {
      mockAnalysis = "GitHubのリモートリポジトリとの同期、または認証キーの設定に問題があります。";
      mockSteps = [
        { text: "git remote -v を実行し、リモートのURLが正しいか（Publicリポジトリか）確認する", timeEstimate: "2分" },
        { text: "git status を実行し、未コミットの変更がないか確認する", timeEstimate: "2分" },
        { text: "git push -u origin main を実行し、エラーが出ないか再確認する", timeEstimate: "3分" }
      ];
    }

    injectErrorTasks(mockAnalysis, mockSteps);
    
    debugText.textContent = '⚡️ 解決手順を割り込ませる';
    debugErrorBtn.disabled = false;
    if (debugLoader) debugLoader.classList.add('hidden');
    errorInput.value = '';
    return;
  }
  ...
  // 2. Live API Request
  try {
    const parentTaskText = taskInput.value.trim();
    const headers = { 'Content-Type': 'application/json' };
    if (appState.customKey) {
      headers['x-api-key'] = appState.customKey;
    }
    ...
    injectErrorTasks(data.errorAnalysis, data.solutionSteps);
    errorInput.value = '';
  } catch (err) {
    console.error('Debug error failed:', err);
    alert(`エラー解決手順の生成に失敗しました: ${err.message}`);
  } finally {
    debugText.textContent = '⚡️ 解決手順を割り込ませる';
    debugErrorBtn.disabled = false;
    if (debugLoader) debugLoader.classList.add('hidden');
...
function injectErrorTasks(analysis, steps) {
  // Show analysis result
  errorAnalysisText.textContent = analysis;
  errorAnalysisResult.classList.remove('hidden');

  // Convert step objects and inject them at the beginning of roadmap
  const newTasks = steps.map((step, index) => ({
    id: `task-error-${Date.now()}-${index}`,
    text: `[エラー対処] ${step.text}`,
    timeEstimate: step.timeEstimate || '5分',
    completed: false,
    completionReason: '',
    substeps: [],
  ...
  // Put error tasks at the top of the roadmap
  appState.roadmap = [...newTasks, ...appState.roadmap];
...
function renderRoadmap() {
  ...
  appState.roadmap.forEach((task) => {
    ...
    if (task.isInjectedError) {
      li.classList.add('injected-error-task');
    }
    ...
    if (task.isInjectedError) {
      ...
      errorBadge.className = 'injected-error-badge';
      errorBadge.textContent = 'エラー対処';
      errorBadge.style.marginLeft = '0.5rem';
